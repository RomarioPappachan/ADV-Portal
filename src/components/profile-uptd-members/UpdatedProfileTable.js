...
function UpdatedProfileTable() {
  const { resetSelectedMember } = useMemberStore();
  const { updatedMembers, loading, getUpdatedMembers } =
  ...
  useEffect(() => {
    if (date) getUpdatedMembers(date);
  }, [date]);
...
export default UpdatedProfileTable;
